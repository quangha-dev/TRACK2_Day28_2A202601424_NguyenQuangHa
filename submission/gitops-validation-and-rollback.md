...
Trước khi deploy, phải chọn revision đã push của fork và image chứa đúng mã bài làm. Revision phải được pin vào commit hoặc release tag tồn tại. Việc đổi image/tag trong Git, sync và kiểm tra pod/gateway là rollback triển khai; chuyển alias champion trong MLflow là rollback release mô hình. Hai thao tác cần được giải thích riêng.
